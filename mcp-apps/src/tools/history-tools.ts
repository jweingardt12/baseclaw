...
export function registerHistoryTools(server: McpServer, enabledTools?: Set<string>) {
  ...
  defineTool(server, "yahoo_league_history", {
    ...
    var data = await apiGet<LeagueHistoryResponse>("/api/league-history");
    var lines = ["League History:"];
    for (var s of data.seasons) {
      var line = "  " + s.year + ": Champion: " + s.champion;
  ...
  // yahoo_record_book
  defineTool(server, "yahoo_record_book", {
  ...
  defineTool(server, "yahoo_past_standings", {
    ...
    var data = await apiGet<PastStandingsResponse>("/api/past-standings", { year: String(year) });
    var lines = ["Standings for " + year + ":"];
    for (var s of data.standings) {
  ...
  // yahoo_past_draft
  defineTool(server, "yahoo_past_draft", {
    ...
    var data = await apiGet<PastDraftResponse>("/api/past-draft", { year: String(year), count: String(count) });
    var lines = ["Draft " + year + ":"];
  ...
  // yahoo_past_teams
  defineTool(server, "yahoo_past_teams", {
    ...
    var data = await apiGet<PastTeamsResponse>("/api/past-teams", { year: String(year) });
    var lines = ["Teams for " + year + ":"];
  ...
  // yahoo_past_trades
  defineTool(server, "yahoo_past_trades", {
    ...
    var data = await apiGet<PastTradesResponse>("/api/past-trades", { year: String(year), count: String(count) });
    var lines = ["Trades for " + year + ":"];
  ...
  // yahoo_past_matchup
  defineTool(server, "yahoo_past_matchup", {
    ...
    var data = await apiGet<PastMatchupResponse>("/api/past-matchup", { year: String(year), week: String(week) });
    var lines = ["Matchups " + year + " Week " + week + ":"];
    for (var m of data.matchups) {
  ...
  defineTool(server, "yahoo_roster_history", {
    ...
    if (team_key) params.team_key = team_key as string;
    if (!week && !date) {
      throw new Error("Provide either week or date parameter");
    }
    var data = await apiGet<RosterHistoryResponse>("/api/roster-history", params);
    var players = data.players || [];
    var lines = ["Roster for " + data.label + ":"];
    for (var p of players) {
      var line = "  " + str(p.position).padEnd(4) + " " + str(p.name).padEnd(25) + " " + (p.eligible_positions || []).join(",") + pid(p.player_id);
