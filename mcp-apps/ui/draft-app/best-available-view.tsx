...
export function BestAvailableView({ data, app, navigate }: { data: BestAvailableData; app?: any; navigate?: (data: any) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {app && (
        <Tabs value={data.pos_type || "B"} onValueChange={handleTypeChange} className="mb-2">
          <TabsList>
      ...
      <div className="flex gap-1 flex-wrap mb-2">
        {POSITION_FILTERS.map(function (pos) {
          return (
            <Badge
              key={pos}
              variant={posFilter === pos ? "default" : "secondary"}
          ...
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <Badge variant="secondary">{topPlayer.positions ? topPlayer.positions.join(", ") : (topPlayer.position || "?")}</Badge>
              </div>
            </div>
